...
public class DefaultInput extends InputProvider {
	...
	public void poll() {
		...
		while(Mouse.next()) {
			...
			Triple<Integer, Boolean, Duple<Integer, Integer>> t = 
					new Triple<Integer, Boolean, Duple<Integer, Integer>>(k, down, new Duple<Integer, Integer>(Mouse.getEventX(), Mouse.getEventY()));
			mouseEvents.add(t);
			if(down)
